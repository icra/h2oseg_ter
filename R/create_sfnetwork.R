...
for (i in 1:nrow(edges)){
  from <- edges$from[i]
  to <- edges$to[i]
  line <- rbind(st_coordinates(nodes[nodes$node_id == from, ]), st_coordinates(nodes[nodes$node_id == to, ])) |> 
    st_linestring()
...
edges_sf$codi <- masses$EUMSPFCOD[st_nearest_feature(st_centroid(edges_sf), masses)]
...
plot(st_geometry(multi_sf), col = "blue", lwd = 2, main = "Multilínia en forma de Y")
